...
async def main():
    ...
    # Создание бота и диспетчера с оптимизированными настройками
    bot = Bot(
        token=BOT_TOKEN,
    ...
    try:
        # Удаляем вебхук если был установлен с увеличенным таймаутом
        try:
        ...
        # Получаем информацию о боте с retry логикой
        try:
            bot_info = await bot.get_me()
            logger.info(f"✅ Бот запущен: @{bot_info.username} (ID: {bot_info.id})")
        except Exception as e:
            logger.warning(f"⚠️ Не удалось получить информацию о боте: {e}")
            # Продолжаем работу даже без информации о боте
        ...
        await dp.start_polling(
            ...
            close_bot_session=False  # Оставляем сессию открытой при остановке
        )
